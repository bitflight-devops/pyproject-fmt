...
Plans:
- [x] 02-01-PLAN.md -- CLI commands with fix/check/diff modes, file and stdin I/O, error handling, comprehensive test suite
- [x] 02-02-PLAN.md -- Configuration loading from [tool.pyproject-fmt], extend/replace merging, conflict detection, config test suite
...
### Phase 3.1: Golden File Correction (INSERTED)
**Goal**: Restore the user-provided golden file (after.toml), identify all pipeline deviations from the specification, and fix the pipeline so its output matches the golden file byte-for-byte.
**Depends on**: Phase 3
**Requirements**: TEST-01, TEST-02, TEST-06 (re-validation — these were invalidated by golden file contamination)
**Success Criteria** (what must be TRUE):
